...
<?php
	...
	$today = date('Y-m-d');
	$res = mysql_query("SELECT h.*, p.*, d.pb1, SUM(d.servCharge5) AS servCharge5, v.voucherName, v.voucherRequirement, v.voucherSaldo, v.description voucherDesc FROM tabOrderHeader h 
					...
					<div class='col-lg-4'>
						<div class='leftist'>
							...
							<div class='row mb-2'>
								<div class='col'>
									<?php 
										echo "<input type='text' id='customerID' name='customerID' value=$row[payerMemberID]>";
									?>
							...
							<?php								
								...
								$resultD = mysql_query("SELECT p.productName, d.* FROM mProduct p INNER JOIN tabOrderDetail d ON p.productID = d.productID WHERE d.orderID = '$orderID' AND d.status = 0
															UNION ALL
														SELECT b.bundleName, d.* FROM tabBundleHeader b INNER JOIN tabOrderDetail d ON b.bundleID = d.productID WHERE d.orderID = '$orderID' AND d.status = 0");
									...
									echo"
										<tr id='additem$rows[id]'>
												...
												<td class = 'table-details' style='text-align:right;width:29%;padding-right:5px;'>
													...
													<input type='hidden' name='ori_servCharge5[]' value='".($rows[servCharge5] / $rows[productAmount])."'/>
							...
							<div class='row mb-2'>
								<div class='col'>
									<select class="form-control" id ='orderMethod' name ='orderMethod' style='width:100%;'>
										...
										<option value='3'>ONLINE/DELIVERY</option>
									</select>
								</div>
							</div>
					...
					<div class='col-lg-8'>
						<div class='container leftist' id='productList'>
							...
							<?php
								...
								$resMenu = mysql_query("SELECT * FROM tabBundleHeader WHERE outletID = '$outletID' AND status = 1");
								while($rowMenu = mysql_fetch_array($resMenu)){
									$bundleID = $rowMenu['bundleID'];
								echo"
									<div class='d-flex ml-1 productEntry'>
										<div class='pt-2'>
											<div class='col-12'>
											    <div class='card' id='bundles'>
									        		<button type='button' class='menu d-flex' id='$rowMenu[bundleID]'>
									            <div class=''>
									                <img  alt='$rowMenu[bundleName]' width='50px' height='50px'>	
									            </div>
									            <div class='col-6'>
									        	    <span>$rowMenu[bundleName]</span><input type='hidden' id = 'curProd_$rowMenu[bundleID]' value='$rowMenu[curStock]'/> 
									            </div>
									            <div class='col-3'>
													<span>Stock : <span/><input type='text' readonly class='form-control-sm form-control-plaintext' id ='stock_$rowMenu[bundleID]' style='text-align:center;font-weight:bold;vertical-align:top;' value='$rowMenu[curStock]'/>
									            </div>
									        		</button>
									   			</div>
											</div>
		...
		$( 'input[name^=productQty]' ).each( function( i , e ) {
			...
			if ( !isNaN( v ) ){
				...
				if(prodID==productID){
					curSell += v;
					if(v != 0){
						$rows.find('input[name^=servCharge5]').val(parseInt(_servCharge5) * v ).trigger('change');
						$rows.find('input[name^=ori_servCharge5]').val(parseInt(_servCharge5) ).trigger('change');
						$rows.find('input[name^=pb10]').val(parseInt(_pb1) * v ).trigger('change');
						$rows.find('input[name^=ori_pb1]').val(parseInt(_pb1) * v ).trigger('change');
