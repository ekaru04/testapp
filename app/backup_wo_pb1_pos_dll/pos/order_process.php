	...
	if($status == 0){
		...
		if($countArr!=0){
			...
			for($x=0;$x<$countArr;$x++) {
				...
				$id = $x+1;
				$queryD = mysql_query("INSERT INTO taborderdetail(id,orderID,productID,productAmount,productPrice,servCharge5,pb1,productSubtotal,status,dateCreated,lastChanged)VALUES('$id','$orderID', '$product', '$amount', '$price', '$servCharge5[$x]', '$pb1[$x]', '$subtotal', 0, '$dateCreated', '$lastChanged')");
			}
		...
		if($countArr!=0){
			...
			for($x = 0;$x<$countArr;$x++) {
				...
				/* Insert product ke DB OrderDetail */
				$id = $x+1;
				$queryD = "INSERT INTO taborderdetail(id,orderID,productID,productAmount,productPrice,servCharge5,pb1,productSubtotal,status, dateCreated,lastChanged)VALUES('$id','$orderID', '$product', '$amount', '$price', '$servCharge5[$x]', '$pb1[$x]', '$subtotal', 1, '$dateCreated', '$lastChanged')";
