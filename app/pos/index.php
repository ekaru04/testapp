				...
				<div class='row'>
						...
						<div class='leftist'>							
							...
							<div class='row mb-2'>
								<div class='col'>
									...
									<select class="mb-2 form-control" name='customerID' id="listCust">
										<option value="">-List Pelanggan-</option>
										<?php
										$queryCust = mysql_query("SELECT customerID, customerName FROM mCustomer UNION ALL SELECT employeeID, employeeName FROM mEmployee ");
											while($rowCust = mysql_fetch_array($queryCust)){
												echo "<option value='$rowCust[customerID]'>$rowCust[customerID] - $rowCust[customerName]</option>";
											}
										?>
									</select>
					...
					<div class='col-lg-8'>
						<div class='container leftist' id='productList'>
							...
							<div class='row'>
							<?php
								$resMenu = mysql_query("SELECT *, IF(recipeType = 2, 100, curStock) as curStock FROM mProduct WHERE outletID = '$outletID' AND status = 1");
									...
									<div class='d-flex ml-1 productEntry'>
										<div class='pt-2'>
											<div class='col-12'>
									            ...
									            <div class=''>
									                <img src='../../productImages/$rowMenu[bundleImage]' alt='$rowMenu[bundleName]' width='50px' height='50px'>	
									            </div>
									            <div class='col-6'>
   ...
   $("#categorySel").change(function(){
    ...
    $(document).on("click", ".menu", function(){
		...
		if(orderType.length <= 0){
			return alert("Silahkan pilih tipe order terlebih dahulu");
		}
	...
	$('body').on('click', '.del', function(event){
		...
		if(stockAfter<0){
			...
			var totalProduct = totProd;
			var grossPrice = dpp;
			var totalPrice = grand+totalServCharge5;
			var discounts = disc;
			var changeVal = change;
	...
	$(document).on('change', '#paymentMethod', function(){
		...
		if(paymentMethod != 'CASH' && paymentMethod != 'JATAH'){
			$('#payment').val(totalPrice);
			$('#payAmount').val(totalPrice);
