...
<?php
	$preorderID = $_GET[preorderID]; //ambil outlet dari session sesuai yang login
	$outletID = $_SESSION[outletID];

	$today = date('Y-m-d');
	$res = mysql_query("SELECT h.*, p.*, v.voucherName, v.voucherRequirement, v.voucherSaldo, v.description voucherDesc FROM tabpreorderheader h 
		INNER JOIN tabpaymentorder p ON h.preorderID = p.orderID
		LEFT JOIN mVoucher v ON p.voucherID = v.voucherCode WHERE preorderID = '$preorderID'");
	...
	$resD = mysql_query("SELECT max(id) max FROM tabpreorderdetail d WHERE d.preorderID = '$preorderID' AND d.status = 0");
	$rowD = mysql_fetch_array($resD);
	$itemCount = $rowD[max];

	/* Ambil ID Customer dari DB */
	$cust = mysql_query("SELECT * FROM tabpreorderheader h INNER JOIN mcustomer c ON h.payerName = c.customerName WHERE h.payerName = '$row[payerName]'")or die(mysql_error());
				...
				<div class='row'>
					<div class='col-4'>
						<div class='leftist'>
							<div class='row mb-2'>
								...
								<div class='col-3'>
									<input type='text' readonly class="form-control-plaintext" id ='preorderNo' name ='preorderNo' style='margin-left:-30px;width:100%;font-weight:bold;' value='<?php echo $preorderNo;?>'/>
								</div>
								<div class='col-3'>
									<input type='text' readonly class="form-control-plaintext" style='margin-left:50px;width:50%;' value='Date:'/>
								</div>
								<div class='col-3'>
									<input type='date' readonly class="form-control-plaintext" id ='orderDate' name ='orderDate' style='float:right;text-align:right;width:180%;font-weight:bold;margin-left:-60px;margin-right:-20px;' value='<?php echo $row[preorderDate];?>'/>
								</div>
							</div>
							<div class='row mb-2'>
								<div class='col'>
							...
							<div class='row mb-2'>
								<div class='col'>
									<div class='row'>
										...
										<div class='col-3'>
											<input type='text' readonly class="form-control-sm form-control-plaintext" id ='totalProduct'  style='float:right;text-align:right;width:80px;font-weight:bold;vertical-align:top;' value='<?php echo $row[preorderAmount];?>'/>
											<input type='hidden' name='totalProduct' id='totalProd' value='<?php echo $row[preorderAmount];?>'/>
										</div>
										<div class='col-2'>
											<b>Total</b>
										</div>
					...
					<div class='col-8'>
						<div class='container rightist'>
							...
							<div class='row'>
								<div class='col'>
									<h5 style='margin-bottom:-20px;'>$rowCat[categoryName]</h5>
								</div>
							</div>
							<div class='row d-flex p-3'>";
								
								$resMenu = mysql_query("SELECT * FROM mProduct WHERE outletID = '$outletID' and categoryID = '$rowCat[categoryID]' AND status = 1");
								while($rowMenu = mysql_fetch_array($resMenu)){
									$amountSold = $rowMenu[curStock];
									$curProd = $rowMenu[curStock];
									$resAmount = mysql_query("select d.productID,sum(d.productAmount) amount from tabpreorderdetail d where preorderID = '$preorderID' AND d.status = 0 group by d.productID");
									while($rowAmount = mysql_fetch_array($resAmount)){
										if($rowAmount[productID]==$rowMenu[productID]){
											$amountSold = $rowMenu[curStock]-$rowAmount[amount];
										}
									}
									echo"
								<div class='p-2'>
									<div class='card'>
										<button type='button' class='menu' id='$rowMenu[productID]'>
											<img src='../../productImages/$rowMenu[productImage]' alt='$rowMenu[productName]' width='100px' height='100px'>
											<br/><b>$rowMenu[productName]</b><input type='hidden' id = 'curProd_$rowMenu[productID]' value='$curProd'/>
											<br/><b>Stock : <b/><input type='text' readonly class='form-control-sm form-control-plaintext' id ='stock_$rowMenu[productID]' style='text-align:center;width:100px;font-weight:bold;vertical-align:top;' value='$amountSold'/>
										</button>
									</div>
								</div>
									";
