<?php
	...
	session_start();
	include("./recipe_product_process.php");
...
$fetchinLoyalPrimary = mysql_fetch_array($queryIDLoyal);
$requirePrimary = $fetchinLoyalPrimary['requirement'];
$pointPrimary = $fetchinLoyalPrimary['point'];
 ...
 */
	...
	if($status == 0){
			...
			for($x = 0;$x<$countArr;$x++) {
				...
				if(substr($product,0,3)=="BDN"){
					...
					while ($rowBundle = mysql_fetch_array($checkBundle)) {
						...
						else if($rowGetProduct[recipeType] == 2){
							//Cek lagi bahan-bahannya
							$testProduct = productStockTest(array(array('product' => $rowGetProduct[productID], 'recipeID' => $rowGetProduct[recipeID], 'amount' => ($rowBundle['amount'] * $amount))));
							foreach($testProduct as $_testProduct){
								$unavailableMessages[] = $_testProduct;
							}
						}
					}
				}
				else{
					//recipeType = 2 gak bakal ngurangi stok, tapi bahannya yang bakal kekurangin
					$checkPro = mysql_query("SELECT *, IF(recipeType = 2, 100, curStock) as curStock FROM mproduct WHERE productID = '$product' AND outletID = '$outletID'");
					$rowPro = mysql_fetch_array($checkPro);

					$curStock = $rowPro['curStock'];
					$stock = $curStock-$amount;

					if($stock <= 0 && $rowPro[recipeType] != 2){
						$unavailableMessages[] = "Stok produk: " . $rowPro['productName'] . " tidak mencukupi";
					}
					else{
						$productRecipeBased[] = array('productID' => $rowPro[productID], 'categoryID' => $rowPro[categoryID], 'recipeID' => $rowPro[recipeID], 'outletID' => $rowPro[outletID], 'amount' => $amount);
					}
				}
			}
		}
		...
		$testRecipeProduct = productStockTest($productRecipeBased);
		if(count($testRecipeProduct) > 0){
			echo "<script>var messages; </script>";
			echo "<script>messages = `".implode(";", $testRecipeProduct)."` </script>";
			echo "<script type='text/javascript'>alert(messages.split(';').join('\\r\\n'))</script>";
			$URL="/picaPOS/app/order/manage_order.php?orderID=" . $orderID; 
			echo "<script type='text/javascript'>location.replace('$URL');</script>";
			return;
		...
		if($namePayment == "JATAH") {
				 ...
				 for($x = 0;$x<$countArr;$x++) {
					 ...
					 if(mysql_num_rows($resChk)==0){
						 ...
						 if($productType==1){
							 ...
							 if($rowPro[recipeType] == 2){
								processRecipeIngredient(array_merge(array('amount' => $amount), $rowPro), $orderID);
							}
							else{
								$measurement = $rowPro['measurementID'];
								$curStock = $rowPro['curStock'];
						 ...
						 if ($productType == 2) {
							 ...
							 while ($rowBundle = mysql_fetch_array($checkBundle)) {
								 ...
								 if($rowGetProduct[recipeType] == 2){
									processRecipeIngredient(array_merge(array('amount' => $amount), $rowGetProduct), $orderID);
								 }
								 else{
									$stockFromProduct = $rowGetProduct['curStock'];
									$updateFinalStock = $stockFromProduct - $totStockProdFromBundle;
			...
			if(strlen($payerName) > 0){
				...
				if(mysql_num_rows($checkLoyalty)==0)
				{
					...
					if($total > $requirePrimary)
					{
						$ql = mysql_query("INSERT INTO tabloyalty(loyaltyID, customerID, outletID, loyaltyPoint, status, dateCreated, lastChanged)VALUES('$idl', '$cusUniqID', '$outletID', '$pointPrimary', 1, '$dateCreated', '$lastChanged')");
					}
					else{
						$ql = mysql_query("INSERT INTO tabloyalty(loyaltyID, customerID, outletID, loyaltyPoint, status, dateCreated, lastChanged)VALUES('$idl', '$cusUniqID', '$outletID', 0, 1, '$dateCreated', '$lastChanged')");
					}
				...
				else
				{
					...
					if($total > $requirePrimary):
						$finalPoint = $loyaltyP + $pointPrimary;
						$ql = mysql_query("UPDATE tabloyalty SET loyaltyPoint = '$finalPoint', lastChanged = '$lastChanged' WHERE customerID = '$payerID'");
