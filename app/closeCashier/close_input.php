...
if($_GET['closeID']==""){
    ...
    $query = mysql_query("SELECT c.*,u.fullName FROM tabCloseCashierHeader c INNER JOIN mUser u ON u.username = c.username WHERE c.closeID ='$closeID'");
...
?>
					...
					<div class='row  mt-1 '>
                        ...
                        <div class='col-4'>
							...
							<input type='text' name='finalOwn' id = 'finalOwn' value='<?php echo $totalReceived; ?>'>
                        </div>
					</div>
...
<script type="text/javascript">
	...
	$("#generate").click(function(){
		...
		var get = "detail_sum.php?closeID="+closeID+"&closeDate="+closeDate+"&outletID="+outletID;
	...
	$("#calculate").click(function(){
		var modalTot =  isNaN($('#modalTot').val()) ||$('#modalTot').val().length == 0  ? 0 : parseFloat($('#modalTot').val());
		var out = isNaN($('#outTot').val()) ||$('#outTot').val().length == 0  ? 0 : parseFloat($('#outTot').val()); 
		var newMod = isNaN($('#newModal').val()) ||$('#newModal').val().length == 0  ? 0 : parseFloat($('#newModal').val());
		var grandTot = isNaN($('#grandTot').val()) ||$('#grandTot').val().length == 0  ? 0 : parseFloat($('#grandTot').val());
		var received = isNaN($('#received').val()) ||$('#received').val().length == 0  ? 0 : parseFloat($('#received').val());;
		...
		console.log("grand Total", $('#grandTot').val());
